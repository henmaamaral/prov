...
export async function salvarPontuacao(nome, pontuacao, tempo) {
    const query = `INSERT INTO ranking (nome_jogador, pontuacao, tempo_segundos) VALUES ($1, $2, $3) RETURNING *;`;
    const resultado = await executarQueryNeon(query, [nome, pontuacao, tempo]);
    return resultado !== null && resultado.length > 0;
}


export async function buscarLeaderboard() {
    const query = `SELECT nome_jogador, pontuacao, tempo_segundos FROM ranking ORDER BY pontuacao DESC LIMIT 10;`;
